...
@Component("HKUtils")
public class HKUtils implements ManufacturerUtils {
	/**
	 * 获取机构的标准ID
	 * 
	 * @param organ
	...
	public String getOrganStdId(Organ organ) {
		if (StringUtils.isBlank(organ.getStdId())) {
			organ.setStdId(getOrganStdIdByCmsId(organ.getCmsId()));
			// Ar.update(organ);
		}
	...
	private synchronized String getOrganStdIdByCmsId(String cmsId) {
		List list = Ar.sql(
				"select max(substr(std_id,7,2)) from organ where cms_id=?",
				cmsId);
		if (list.get(0) == null) {
	...
	public void setItemParentID(Item item, String parentId) {
		// item.setParentID(this.getDeviceID()); //一所说这个不能设置，只能设置这空串
	}
}
